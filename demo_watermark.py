...
from transformers import (AutoTokenizer,
                          ...
                          LogitsProcessorList, BitsAndBytesConfig, AutoModel, AutoTokenizer)

from watermark_processor_multi_bit import WatermarkLogitsProcessor, WatermarkDetector
...
def load_model(args):
    ...
    if args.is_seq2seq_model:
        model = AutoModelForSeq2SeqLM.from_pretrained(args.model_name_or_path, device_map='auto', quantization_config=bnb_config, trust_remote_code=True)
    elif args.is_decoder_only_model:
        if args.load_fp16:
            model = AutoModelForCausalLM.from_pretrained(args.model_name_or_path, device_map='auto', quantization_config=bnb_config, trust_remote_code=True)
        else:
            model = AutoModelForCausalLM.from_pretrained(args.model_name_or_path, device_map='auto', quantization_config=bnb_config, trust_remote_code=True)
    else:
...
def generate(prompt, args, model=None, device=None, tokenizer=None):
    ...
    watermark_processor = WatermarkLogitsProcessor(num_colors=args.num_colors,
    ...
    tokd_input = tokenizer(prompt, return_tensors="pt", add_special_tokens=True,
                           truncation=True, max_length=args.prompt_max_length).to(device)
...
def detect(input_text, args, device=None, tokenizer=None):
    ...
    watermark_detector = WatermarkDetector(vocab=list(tokenizer.get_vocab().values()),
                                           ...
                                           message_len=len(args.message),  # gamma=args.gamma,
                                           seeding_scheme=args.seeding_scheme,
                                           device=device,
                                           tokenizer=tokenizer,
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            with gr.Column(scale=9):
                gr.Markdown(
                    """
                ## 💧 Robust Semantics-based Multi-bit Watermark Demo 🔍
                """
                )
        ...
        with gr.Accordion("Advanced Settings", open=False):
            with gr.Row():
                with gr.Column(scale=1):
                    ...
                    with gr.Row():
                        decoding = gr.Radio(label="Decoding Method", choices=["multinomial", "greedy"], value=(
                            "multinomial" if args.use_sampling else "greedy"))
                    with gr.Row():
                        sampling_temp = gr.Slider(label="Sampling Temperature", minimum=0.1, maximum=1.0,
                                                  step=0.1, value=args.sampling_temp, visible=True)
                    with gr.Row():
                    ...
                    with gr.Row():
                        max_new_tokens = gr.Slider(label="Max Generated Tokens", minimum=10,
                                                   maximum=1000, step=10, value=args.max_new_tokens)
                ...
                with gr.Column(scale=1):
                    ...
                    with gr.Row():
                        detection_z_threshold = gr.Slider(label="z-score threshold", minimum=0.0,
                                                          maximum=10.0, step=0.1, value=args.detection_z_threshold)
                    with gr.Row():
                        ignore_repeated_bigrams = gr.Checkbox(label="Ignore Bigram Repeats")
                    with gr.Row():
                        normalizers = gr.CheckboxGroup(label="Normalizations", choices=[
                                                       "unicode", "homoglyphs", "truecase"], value=args.normalizers)
        ...
        generate_btn.click(fn=generate_partial, inputs=[prompt, session_args], outputs=[
                           redecoded_input, truncation_warning, output_without_watermark, output_with_watermark, session_args])
        # Show truncated version of prompt if truncation occurred
        redecoded_input.change(fn=truncate_prompt, inputs=[redecoded_input,
                               truncation_warning, prompt, session_args], outputs=[prompt, session_args])
        # Call detection when the outputs (of the generate function) are updated
        # output_without_watermark.change(fn=detect_partial, inputs=[output_without_watermark, session_args], outputs=[
        #                                without_watermark_detection_result, session_args])
        # output_with_watermark.change(fn=detect_partial, inputs=[output_with_watermark, session_args], outputs=[
        #                             with_watermark_detection_result, session_args])
        ...
        ignore_repeated_bigrams.change(update_ignore_repeated_bigrams, inputs=[
                                       session_args, ignore_repeated_bigrams], outputs=[session_args])
        ...
        # detection_z_threshold.change(fn=detect_partial, inputs=[output_without_watermark, session_args], outputs=[
        #                             without_watermark_detection_result, session_args])
        # detection_z_threshold.change(fn=detect_partial, inputs=[output_with_watermark, session_args], outputs=[
        #                             with_watermark_detection_result, session_args])
        ...
        # ignore_repeated_bigrams.change(fn=detect_partial, inputs=[output_without_watermark, session_args], outputs=[
        #                               without_watermark_detection_result, session_args])
        # ignore_repeated_bigrams.change(fn=detect_partial, inputs=[output_with_watermark, session_args], outputs=[
        #                               with_watermark_detection_result, session_args])
        ...
        # select_green_tokens.change(fn=detect_partial, inputs=[output_without_watermark, session_args], outputs=[
        #                           without_watermark_detection_result, session_args])
        # select_green_tokens.change(fn=detect_partial, inputs=[output_with_watermark, session_args], outputs=[
        #                           with_watermark_detection_result, session_args])
        # select_green_tokens.change(fn=detect_partial, inputs=[detection_input, session_args], outputs=[detection_result, session_args])
    # demo.queue()

    if args.demo_public:
        demo.launch(share=True, max_threads=20)  # exposes app to the internet via randomly generated link
    else:
